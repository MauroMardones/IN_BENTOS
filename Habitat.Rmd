...
draga_labels <- draga_sf %>%
  filter(Estacion %in% c("249", "232", "234"))
...
dragayoli <- ggplot() +
  ...
  xlab(expression(paste(Longitude^o,~'O'))) +
  ylab(expression(paste(Latitude^o,~'S'))) +
  geom_sf(data = draga_labels, color = "black", size = 1) +  # puntos visibles
  geom_text_repel(data = draga_labels,
                  aes(label = Estacion, geometry = geometry),
                  stat = "sf_coordinates",
                  size = 3,
                  color = "black",
                  nudge_x = -0.05,         # desplaza ligeramente a la izquierda
                  segment.color = "black",
                  segment.size = 0.4,
                  max.overlaps = Inf) +    # permite que siempre intente mostrar
  theme_few() +
  xlim(-7.6, -6.3) +
  ylim(36.65, 37.3)
