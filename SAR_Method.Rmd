...
hist(cajassep$DIA)
hist(cajassep$MES)
hist(cajassep$N_VELOCIDAD)
hist(cajassep$N_RUMBO)
```
...
unique(cajasfilp$N_EN_PUERTO)
hist(cajasfilp$N_VELOCIDAD)
```

3- Calcular el intervalo de tiempo entre registros sucesivos, 


4- Diferenciar entre registros de pesca y no pesca basándose en la velocidad 
