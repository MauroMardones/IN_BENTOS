...
maseffor <- ggplot() +
  ...
  geom_sf(data = grillasar2 %>% filter(!is.na(SAR)), 
          aes(fill = SAR_cat)) +
...
colores <- RColorBrewer::brewer.pal(n = length(levels(sumest$lances_cat)), "Greens")
...
masrend1 <- ggplot() +
  ...
  # Dibujar la capa base con la costa
  geom_sf(data = costandalucia1, fill = "#fee8c8") +
  
  # Dibujar las estaciones con color según la categoría de num_lances
  geom_sf(data = sumest2, aes(fill = lances_cat), color = "black") +
  
  # Escala de colores categórica con cuadrados en la leyenda
  scale_fill_manual(values = colores, name = "N Hauls") +
  #facet_wrap(.~ANO)+
  
  # Añadir etiquetas con el número de lances en cada celda
  #geom_sf_text(data = sumest, aes(label = num_lances), color = "black", size = 3) +
  
  # Coordenadas y etiquetas de ejes
  coord_sf() +
  xlab(expression(paste(Longitude^o,~'O'))) +
  ylab(expression(paste(Latitude^o,~'S'))) +
