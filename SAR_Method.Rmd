...
datos
datosbar <-ggplot(datos, aes(MANIOBRA))+
  geom_bar()
...
datosfil <- datos1 %>% 
  ...
  mutate(VELONUE = N_VELOCIDAD*0.51444, # de nudos a mts/seg
         ...
         AREA = distancia*5,
         SA=distancia*AREA) # como ejemplo 5 mt de aperttura de draga (corroborar)
```
...
plottiem <- ggplot(datosfil %>% 
               group_by(MATRICULA))+
  geom_boxplot(aes(x = reorder(MATRICULA, TIEMPO), 
                   y = TIEMPO),
               outliers = FALSE)+
...
plotsa <- ggplot(datosfil %>% 
               group_by(MATRICULA))+
  geom_boxplot(aes(x = reorder(MATRICULA, SA), 
                   y = SA),
               outliers = FALSE)+
  geom_hline(yintercept = 10, 
             col="red")+
  ...
  theme(axis.text.x = element_text(angle = 90,
                                     ...
                                     size = 5))+
  xlab("Matriculas") +
  ylab(expression(paste(SA (m2))))

ggarrange(plottiem, plotvel, plotsa, ncol =1)
```
Calculo el SAR
